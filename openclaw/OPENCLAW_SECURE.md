...
[orchestrator]  openclaw-gateway
                  ...
                  ├── DISCORD_BOT_TOKEN (from root .env DISCORD_TOKEN) / TELEGRAM_BOT_TOKEN ← channel bots when configured
...
2. `openclaw.json` is never mounted in browser-tier containers. When `DISCORD_TOKEN` / `TELEGRAM_BOT_TOKEN` are set in the project `.env`, `openclaw-config-sync` runs `openclaw/scripts/merge_gateway_config.py` so Discord/Telegram entries use env-backed **SecretRef** in JSON instead of plaintext tokens. Skill or plugin API keys may still appear in JSON where OpenClaw requires it.
...
- Channel bots: `DISCORD_BOT_TOKEN` (from root `.env` `DISCORD_TOKEN`) and optional `TELEGRAM_BOT_TOKEN` are passed to `openclaw-gateway` only; prefer SecretRefs in `openclaw.json` (applied by `merge_gateway_config.py` when those env vars are set) so tokens are not plaintext on disk
- `data/openclaw/openclaw.json` (model routing, plugins, optional skill keys) is only mounted in `openclaw-gateway`
