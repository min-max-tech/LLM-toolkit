...
- **Long-context tuning:** `openclaw-config-sync` also applies `OPENCLAW_COMPACTION_MODE`, `OPENCLAW_AGENT_TIMEOUT_SECONDS`, `OPENCLAW_LLM_IDLE_TIMEOUT_SECONDS`, `OPENCLAW_BOOTSTRAP_MAX_CHARS`, and `OPENCLAW_BOOTSTRAP_TOTAL_MAX_CHARS` from the root `.env`. For large local models, `safeguard` compaction plus a higher idle timeout prevents long prompt-prefill runs from timing out before the first token. Bootstrap caps keep workspace files from dominating `/new` and first-turn prompts. Additionally, model-gateway enforces `OPENCLAW_MAX_INPUT_TOKENS` (default 16384 estimated tokens) to trim oversized prompts before they reach llama.cpp, preventing slow decode regardless of the model's full context window.
